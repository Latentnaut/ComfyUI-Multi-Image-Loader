  function openCropEditor(startIdx, skipAnim, initialMode) {
    ...
    ptColorRow.style.cssText = `display:none;position:relative;width:40px;height:40px;margin:2px 0 0 2px;`;
      ...
      if (edPixelTool && _edBrushDrawing) {
        ...
        if (edPixelTool === "blur") {
          ...
          if (prevPt) {
            ...
            ctx.arc(epx, epy, rPx, 0, Math.PI * 2); ctx.fill();
          }
    ...
    function onKey(e) {
      ...
      if (edPanelMode === "pixels" && !e.ctrlKey && !e.metaKey && !e.altKey) {
        ...
        if (k === "b") { _selectPixelTool("brush"); e.preventDefault(); }
        if (k === "i") { _selectPixelTool("eyedropper"); e.preventDefault(); }
