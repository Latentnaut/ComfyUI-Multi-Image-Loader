...
// Column count per grid_columns preset — images grow freely to fill the node width
// ─── helpers ─────────────────────────────────────────────────────────────────
...
/** Cross-node image DnD (Layer Mask Painter, Grid Composer, etc.). */
const COMFY_IMAGE_MIME = "application/x-comfy-image";

function isExternalImageDrag(dt) {
  const types = dt?.types ? [...dt.types] : [];
  return types.includes("Files") || types.includes(COMFY_IMAGE_MIME);
}

function parseComfyImagePayload(dt) {
  if (!dt) return null;
  let raw = "";
  try {
    raw = dt.getData(COMFY_IMAGE_MIME);
  } catch {
    return null;
  }
  if (!raw) return null;
...
function viewURLFromRef(ref) {
  if (!ref?.filename && !ref?.name) return "";
...
async function filesFromDataTransfer(dt) {
  if (!dt) return [];
  const ref = parseComfyImagePayload(dt);
  if (ref) {
    const resp = await fetch(viewURLFromRef(ref));
    if (!resp.ok) throw new Error(`Fetch failed: ${resp.status}`);
    const blob = await resp.blob();
    const file = new File([blob], ref.filename || "image.png", {
...
function createWidget(node) {
  ...
  addBlankBtn.innerHTML = "<b>＋</b> Null";
  addBlankBtn.title = "Add a blank grid panel";
  ...
  addBlankBtn.addEventListener("mouseenter", () => {
    addBlankBtn.style.background = "#2a3b4c";
    addBlankBtn.style.borderColor = "#88aaff";
  });
  addBlankBtn.addEventListener("mouseleave", () => {
    addBlankBtn.style.background = "#1a2b3c";
    addBlankBtn.style.borderColor = "#334466";
  });
  addBlankBtn.addEventListener("click", () => {
    pushUndoState();
    // A 1x1 transparent PNG data URL
    const blankDataUrl = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==";
    const blankFilename = "__BLANK__" + Date.now() + "_" + Math.floor(Math.random()*1000) + ".png";
    items.push({ filename: blankFilename, src: blankDataUrl, previewSrc: blankDataUrl });
    persist();
    render();
    setTimeout(() => { if(grid) grid.scrollTop = grid.scrollHeight; }, 10);
  });
  ...
  // viewMode removed — layout is always grid; column count driven by grid_columns
  ...
  // (viewToggleBtn removed — grid_columns now controls column layout)
  ...
  function hasCrop(filename) {
    ...
    return !!(t && (t.ox!==0 || t.oy!==0 || t.scale!==1.0 || t.flipH || t.flipV ||
              ...
              t.imageEditsDataUrl || t.imageEditsFile));
  }
  ...
  async function computeRefDims() {
    ...
    if (masterDims) {
      ...
      const totalPx = mp <= 0 ? masterDims.w * masterDims.h : Math.max(1, mp * 1_000_000);
    ...
    if (ar !== "none") {
      ...
      if (mp <= 0) {
        ...
        totalPx = Math.max(1, mp * 1_000_000);
      }
  ...
  /** Number of columns for the current grid_columns preset. */
  ...
  function render() {
    ...
    items.forEach((item, idx) => {
      ...
      // ── drag-to-reorder + external drop payload (Grid Composer, etc.) ─────
      wrapper.addEventListener("dragstart", (e) => {
        dragSrcIdx = idx;
        wrapper.classList.add("mil-dragging");
        e.dataTransfer.effectAllowed = "copyMove";
        e.dataTransfer.setData("text/plain", String(idx));
        if (!item.is_blank && item.filename && !String(item.filename).startsWith("__BLANK__")) {
          const raw = String(item.filename).replace(/\\/g, "/");
          const parts = raw.split("/");
          const name = parts.pop();
          const subfolder = parts.join("/");
          const payload = JSON.stringify({
            source: "MultiImageLoader",
            name,
            filename: name,
            subfolder,
            type: "input",
          ...
          try {
            e.dataTransfer.setData("application/x-comfy-image", payload);
          } catch (_) {}
        }
      });
  ...
  grid.addEventListener("dragover", (e) => {
    const isExternal = dragSrcIdx === null && isExternalImageDrag(e.dataTransfer);
  ...
  grid.addEventListener("drop", async (e) => {
    ...
    if (isExternal) {
      ...
      try {
        const imageFiles = await filesFromDataTransfer(e.dataTransfer);
        if (imageFiles.length > 0) replaceFileAt(targetIdx, imageFiles[0]);
      } catch (err) {
        console.warn("[MIL] external drop replace failed:", err);
      }
      return;
    }
  ...
  async function renderItemToDataUrl(item, idx) {
    ...
    if (t) {
      ...
      // Lasso mask overlay — skip in Paint mode (lassoIsPaint)
  ...
  dropZone.addEventListener("drop", async (e) => {
    ...
    try {
      ...
      console.warn("[MIL] dropZone drop failed:", err);
    }
  });
  ...
  gridWrapper.addEventListener("drop", async (e) => {
    ...
    try {
      ...
      console.warn("[MIL] gridWrapper drop failed:", err);
    }
  });
  ...
  function openCropEditor(startIdx, skipAnim, initialMode) {
    ...
    function _edSaveEditOpsState() {
      _edOpsUndoStack.push({
        ...
        pixelCvs: _edCvsEditsPx, // canvas ref — no copy needed; crop creates a new one
      });
    ...
    let lassoInvertB = null;
    let ptLassoInvertB = null;
    let ptLassoArLockB = null;
    let ptLassoArInput = null;
    let ptLassoArRow = null;
    let ptLassoOptsGroup = null;
    let cropArInput = null;
    let syncOptsBarContent = function() {};
    ...
    // ── Tool options horizontal bar ──
    const ptOptsBar = document.createElement("div");
    ptOptsBar.style.cssText = `display:flex;flex-shrink:0;align-items:center;background:#141414;border-bottom:1px solid #222;padding:0 ${_pad12};gap:${Math.round(15*uiScale)}px;height:${Math.round(36*uiScale)}px;overflow:hidden;`;
    ...
    // ── Quick Scale & Rotate (top of panel, before crop) ──────────
    function _mkValSliderRow(label, valEl, slider) {
      const row = document.createElement("div");
      row.style.cssText = `display:flex;align-items:center;gap:${_gap5};width:100%;`;
      const lbl = document.createElement("div");
      lbl.textContent = label;
      lbl.style.cssText = `color:#666;font-size:${_fs10};flex-shrink:0;width:${Math.round(36*uiScale)}px;text-align:right;`;
      row.appendChild(lbl); row.appendChild(valEl); row.appendChild(slider);
      return row;
    }
    // Scale slider
    const scaleValEl = document.createElement("div");
    scaleValEl.style.cssText=`background:#1e1e1e;color:#ccc;border:1px solid #333;border-radius:3px;font-size:${_fs10};width:${Math.round(44*uiScale)}px;text-align:center;cursor:text;user-select:none;flex-shrink:0;padding:2px 0;box-sizing:border-box;`;
    const scaleSlider = document.createElement("input");
    scaleSlider.type="range"; scaleSlider.min=0.1; scaleSlider.max=3; scaleSlider.step=0.01; scaleSlider.value=1;
    scaleSlider.style.cssText="flex:1;accent-color:#5a7abf;cursor:pointer;";
    function syncScaleUI() {
      scaleSlider.value = edScale;
      scaleValEl.textContent = (edScale*100).toFixed(0)+"%";
    }
    syncScaleUI();
    scaleValEl.addEventListener("click", () => {
      const inp = document.createElement("input");
      inp.type="number"; inp.min=10; inp.max=300; inp.value=Math.round(edScale*100);
      ...
      scaleValEl.innerHTML=""; scaleValEl.appendChild(inp); inp.focus(); inp.select();
      function commitScale(){
        const v = Math.max(0.1,Math.min(3, (parseInt(inp.value)||100)/100));
        if (Math.abs(v - edScale) > 0.001) _edSaveEditOpsState();
        edScale=v; syncScaleUI(); updLbl(); redraw();
      }
      inp.addEventListener("blur", commitScale);
      inp.addEventListener("keydown", e=>{ if(e.key==="Enter") commitScale(); if(e.key==="Escape") syncScaleUI(); });
    });
    scaleSlider.addEventListener("pointerdown", () => _edSaveEditOpsState(), { once: false });
    scaleSlider.addEventListener("input", () => { edScale=parseFloat(scaleSlider.value); syncScaleUI(); updLbl(); redraw(); });
    secEdit.appendChild(_mkValSliderRow("Scale", scaleValEl, scaleSlider));

    // Rotate slider (top position — same edRotate variable as the Flip/Rotate section below)
    const rotValElTop = document.createElement("div");
    rotValElTop.style.cssText=`background:#1e1e1e;color:#ccc;border:1px solid #333;border-radius:3px;font-size:${_fs10};width:${Math.round(44*uiScale)}px;text-align:center;cursor:text;user-select:none;flex-shrink:0;padding:2px 0;box-sizing:border-box;`;
    const rotSliderTop = document.createElement("input");
    rotSliderTop.type="range"; rotSliderTop.min=-180; rotSliderTop.max=180; rotSliderTop.step=1; rotSliderTop.value=0;
    rotSliderTop.style.cssText="flex:1;accent-color:#5a7abf;cursor:pointer;";
    function syncRotUITop() {
      rotSliderTop.value = edRotate;
      rotValElTop.textContent = edRotate+"\u00b0";
    }
    syncRotUITop();
    rotValElTop.addEventListener("click", () => {
      const inp = document.createElement("input");
      inp.type="number"; inp.min=-180; inp.max=180; inp.value=edRotate;
      inp.style.cssText=`width:100%;height:100%;box-sizing:border-box;background:transparent;color:#ccc;border:none;outline:none;font-size:${_fs10};text-align:center;`;
      rotValElTop.innerHTML=""; rotValElTop.appendChild(inp); inp.focus(); inp.select();
      function commitRotTop(){
        const v = Math.max(-180,Math.min(180,parseInt(inp.value)||0));
        if (v !== edRotate) _edSaveEditOpsState();
        edRotate=v; syncRotUITop(); if(typeof syncRotUI==="function") syncRotUI(); updLbl(); redraw(); requestInpaintPreview();
      }
      inp.addEventListener("blur", commitRotTop);
      inp.addEventListener("keydown", e=>{ if(e.key==="Enter") commitRotTop(); if(e.key==="Escape") syncRotUITop(); });
    });
    rotSliderTop.addEventListener("pointerdown", () => _edSaveEditOpsState(), { once: false });
    rotSliderTop.addEventListener("input", () => {
      edRotate=parseInt(rotSliderTop.value); syncRotUITop(); if(typeof syncRotUI==="function") syncRotUI(); updLbl(); redraw(); requestInpaintPreview();
    });
    secEdit.appendChild(_mkValSliderRow("Rotate", rotValElTop, rotSliderTop));
    ...
    function syncLassoToggle() {
      ...
      if (edLassoMode) {
        ...
        ca.style.cursor = "crosshair";
        hint.textContent = edLassoTool === "freehand" ? "Drag to draw \u00b7 Alt: straight line \u00b7 Shift: add \u00b7 Shift+Alt: subtract" : "Click vertices \u00b7 Alt: ortho-snap \u00b7 Close near start / dblclick \u00b7 Shift: add \u00b7 Shift+Alt: subtract";
      } else {
    ...
    // (lassoActRow moved to top options bar)
    ...
    lassoHintLbl.textContent = "Alt: straight/ortho \u00b7 Shift: add \u00b7 Shift+Alt: subtract";
    ...
    function commitLassoShape(points, e) {
      ...
      let mode = "add";
      if (e && e.altKey && e.shiftKey) mode = "subtract";
      if (!e || !e.shiftKey) {
        edLassoOps = [];
    ...
    let _edActiveStrokeCvs = null;
    let _edEditsUndoStack = [];
    let _edSmudgeBuf = null;
    let _edSmudgeStr = parseFloat(localStorage.getItem("mil_smudge_strength") ?? "0.5");
    if (isNaN(_edSmudgeStr) || _edSmudgeStr < 0 || _edSmudgeStr > 1) _edSmudgeStr = 0.5;
    let _edBlurStr = parseFloat(localStorage.getItem("mil_blur_strength") ?? "0.5");
    if (isNaN(_edBlurStr) || _edBlurStr < 0 || _edBlurStr > 1) _edBlurStr = 0.5;
    ...
    ptLevelsBtn.addEventListener("click", () => openLevelsModal());
    ...
    ptFgPicker.addEventListener("input", (e) => {
      edColorFg = e.target.value;
      localStorage.setItem("mil_fg_color", edColorFg);
      if (typeof ptLassoFillPick !== "undefined") ptLassoFillPick.value = edColorFg;
    });
    ...
    // Hardness row (none in grid loader, but keep slot or mock if needed)

    // ── Lasso/Selection options group ──
    ptLassoOptsGroup = document.createElement("div");
    ptLassoOptsGroup.style.cssText = `display:none;align-items:center;gap:${Math.round(15*uiScale)}px;`;

    // Invert Selection
    ptLassoInvertB = document.createElement("button");
    ptLassoInvertB.innerHTML = `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px;margin-right:4px;"><path d="M3 8h14l-4-4"/><path d="M21 16H7l4 4"/></svg>Invert`;
    ptLassoInvertB.title = "Invert Selection";
    ptLassoInvertB.style.cssText = `background:#1e1e1e;color:#aaa;border:1px solid #333;border-radius:${_r5};padding:${_pad4} ${_pad8};font-size:${_fs10};cursor:pointer;transition:background 0.15s,border-color 0.15s,color 0.15s;display:flex;align-items:center;justify-content:center;height:${Math.round(24*uiScale)}px;`;
    ptLassoInvertB.addEventListener("click", () => {
      _edSaveEditOpsState();
    ...
    // Alias/Hoisted refs
    pxLassoInvertB = ptLassoInvertB;
    lassoInvertB = ptLassoInvertB;

    // Deselect All
    const ptLassoDeselectB = document.createElement("button");
    ptLassoDeselectB.innerHTML = `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px;margin-right:4px;"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>Deselect`;
    ptLassoDeselectB.title = "Deselect All";
    ptLassoDeselectB.style.cssText = `background:#1e1e1e;color:#aaa;border:1px solid #333;border-radius:${_r5};padding:${_pad4} ${_pad8};font-size:${_fs10};cursor:pointer;transition:background 0.15s,border-color 0.15s,color 0.15s;display:flex;align-items:center;justify-content:center;height:${Math.round(24*uiScale)}px;`;
    ptLassoDeselectB.addEventListener("click", () => {
      if (typeof edAppliedCrop !== 'undefined' && edAppliedCrop && edPanelMode !== "pixels") return;
      _edSaveEditOpsState();
    ...
    // Aspect Ratio Lock Row
    ptLassoArRow = document.createElement("div");
    ptLassoArRow.style.cssText = `display:none;align-items:center;gap:${_gap5};`;

    ptLassoArLockB = document.createElement("button");
    ptLassoArLockB.style.cssText = `background:none;border:none;cursor:pointer;font-size:${_fs12};padding:0 2px;flex-shrink:0;transition:opacity 0.15s;line-height:1;`;
    ptLassoArLockB.addEventListener("click", () => { edCropArLock = !edCropArLock; syncArLockUI(); if (edCropArLock) refitCropBoxToAR(); redraw(); });

    ptLassoArInput = document.createElement("input");
    ptLassoArInput.type = "text";
    const _nodeArWidget = getAspectRatioWidget();
    const _nodeArVal = _nodeArWidget?.value;
    ptLassoArInput.value = (_nodeArVal && _nodeArVal !== 'none') ? _nodeArVal : _simplifyAR(edRefW, edRefH);
    ptLassoArInput.placeholder = "16:9";
    ptLassoArInput.style.cssText = `width:${Math.round(60*uiScale)}px;background:#1a1a1a;color:#ccc;border:1px solid #333;border-radius:${_r5};padding:${_pad4};font-size:${_fs11};text-align:center;height:${Math.round(24*uiScale)}px;`;
    ptLassoArInput.addEventListener("focus", () => { ptLassoArInput.style.borderColor = '#5a7abf'; });
    ptLassoArInput.addEventListener("blur", () => { ptLassoArInput.style.borderColor = '#333'; });
    ptLassoArInput.addEventListener("keydown", (e) => {
      if (e.key === "Enter") { e.preventDefault(); ptLassoArInput.blur(); edCropArLock = true; syncArLockUI(); refitCropBoxToAR(); redraw(); }
    });

    // Sync AR lock UI (unified version)
    function syncArLockUI() {
      ptLassoArLockB.textContent = edCropArLock ? "\uD83D\uDD12" : "\uD83D\uDD13";
      ptLassoArLockB.style.opacity = edCropArLock ? "1" : "0.4";
      ptLassoArInput.style.color = edCropArLock ? "#ccc" : "#555";
    }
    syncArLockUI();

    ptLassoArRow.appendChild(ptLassoArLockB);
    ptLassoArRow.appendChild(ptLassoArInput);

    // Fill with Color (Pixels selection tools)
    const ptLassoFillRow = document.createElement("div");
    ptLassoFillRow.style.cssText = `display:none;align-items:center;gap:${_gap5};`;
    const ptLassoFillPick = document.createElement("input");
    ptLassoFillPick.type = "color";
    ptLassoFillPick.value = edColorFg;
    ptLassoFillPick.title = "Fill color";
    ptLassoFillPick.style.cssText = `width:${Math.round(26*uiScale)}px;height:${Math.round(22*uiScale)}px;border:none;background:none;cursor:pointer;padding:0;flex-shrink:0;`;
    ptLassoFillPick.addEventListener("input", (e) => {
      edColorFg = e.target.value;
      localStorage.setItem("mil_fg_color", edColorFg);
      if (typeof ptFgPicker !== "undefined") ptFgPicker.value = edColorFg;
    });
    const ptLassoFillB = document.createElement("button");
    ptLassoFillB.innerHTML = `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px;margin-right:4px;"><path d="m19 11-8-8-8.6 8.6a2 2 0 0 0 0 2.8l5.2 5.2c.8.8 2 .8 2.8 0L19 11Z"/><path d="m5 2 5 5"/><path d="M2 13h15"/><path d="M22 20a2 2 0 1 1-4 0c0-1.6 1.7-2.4 2-4 .3 1.6 2 2.4 2 4Z"/></svg>Fill`;
    ptLassoFillB.title = "Fill selection with color";
    ptLassoFillB.style.cssText = `background:#1e1e1e;color:#aaa;border:1px solid #333;border-radius:${_r5};padding:${_pad4} ${_pad8};font-size:${_fs10};cursor:pointer;transition:background 0.15s,border-color 0.15s,color 0.15s;display:flex;align-items:center;justify-content:center;height:${Math.round(24*uiScale)}px;`;
    ptLassoFillB.addEventListener("mouseenter", () => { ptLassoFillB.style.background="#2a2a2a"; ptLassoFillB.style.borderColor="#555"; });
    ptLassoFillB.addEventListener("mouseleave", () => { ptLassoFillB.style.background="#1e1e1e"; ptLassoFillB.style.borderColor="#333"; });
    ptLassoFillB.addEventListener("click", () => _edApplySelectionFill());
    ptLassoFillRow.appendChild(ptLassoFillPick);
    ptLassoFillRow.appendChild(ptLassoFillB);

    ptLassoOptsGroup.appendChild(ptLassoInvertB);
    ptLassoOptsGroup.appendChild(ptLassoDeselectB);
    ptLassoOptsGroup.appendChild(ptLassoFillRow);
    ptLassoOptsGroup.appendChild(ptLassoArRow);
    ptOptsBar.appendChild(ptLassoOptsGroup);

    // Make cropArInput alias
    cropArInput = ptLassoArInput;

    // Define unified syncOptsBarContent
    syncOptsBarContent = function() {
      const isLasso = edLassoMode;
      const isPx = edPanelMode === "pixels";

      ptLassoOptsGroup.style.display = isLasso ? "flex" : "none";
      if (isLasso) {
        ptLassoArRow.style.display = (edLassoTool === "marquee") ? "flex" : "none";
        ptLassoFillRow.style.display = isPx ? "flex" : "none";
        if (isPx) ptLassoFillPick.value = edColorFg;
      }

      const showPx = !isLasso && isPx;
      ptColorWrapper.style.display = (showPx && (edPixelTool === "brush" || edPixelTool === "eyedropper")) ? "flex" : "none";
      ptAlphaRow.style.display = (showPx && edPixelTool === "brush") ? "flex" : "none";

      // Update Smudge/Blur Strength slider UI value on tool switch
      if (showPx) {
        if (edPixelTool === "smudge") {
      ...
      ptSmudgeRow.style.display = (showPx && ["smudge", "blur"].includes(edPixelTool)) ? "flex" : "none";
      const needsSize = showPx && ["blur", "smudge", "brush"].includes(edPixelTool);
      ptBrushRow.style.display = needsSize ? "flex" : "none";

      const hasPxOptions = ["brush", "blur", "smudge", "eyedropper"].includes(edPixelTool);
      const showOpts = (isPx && hasPxOptions) || isLasso;
    ...
    function syncLassoInvertBtn() {
      if (edLassoInverted) {
        ptLassoInvertB.style.background = "#2a1a3a"; ptLassoInvertB.style.color = "#bb88ff"; ptLassoInvertB.style.borderColor = "#553388";
      } else {
        ptLassoInvertB.style.background = "#1e1e1e"; ptLassoInvertB.style.color = "#aaa"; ptLassoInvertB.style.borderColor = "#333";
      }
    }
    ...
    function _edEnsureEditsPx() {
      ...
      // ── INPAINT FLATTEN: bake server-rendered inpaint preview as editable base ──
      const isInpaintMode = (edBg === "telea" || edBg === "navier-stokes");
      if (isInpaintMode && edInpaintPreview && !edInpaintDirty) {
        // The inpaint preview already contains the fully composited result
        // (transforms + inpaint fill). Draw it stretched to fill the pixel canvas.
        ctx.drawImage(edInpaintPreview, 0, 0, wpX, wpY);
        return;
      }
    ...
    function _edApplyEyedropper(ex, ey) {
      ...
      if (imgData[3] > 0) {
        ...
        if (typeof ptLassoFillPick !== 'undefined') ptLassoFillPick.value = hex;
    ...
    function _edApplyBlur(cx, cy, r) {
      ...
      const rad = Math.max(1, Math.floor(r * 0.5 * _edBlurStr));
      ...
      for (let row = 0; row < bh; row++) {
        for (let col = 0; col < bw; col++) {
          ...
          const f = dist <= r ? Math.pow(1 - dist / r, 1.2) * _edBlurStr : 0;
    ...
    // ── Solid fill inside current selection ───────────────────────
    function _edApplySelectionFill() {
      const hasSel = (typeof edLassoOps !== "undefined" && edLassoOps.length > 0) || !!edLassoInverted;
      if (!hasSel) {
        alert("Draw a selection first, then Fill with Color.");
        return;
      }
      _edEnsureEditsPx();
      if (!_edCvsEditsPx) return;
      ...
      const mask = buildLassoMaskCanvas(pw, ph);
      const tmp = document.createElement("canvas");
      tmp.width = pw; tmp.height = ph;
      const tctx = tmp.getContext("2d");
      tctx.fillStyle = edColorFg;
    ...
    function _edApplyLevels(inBlack, gamma, inWhite, outBlack, outWhite) {
      inBlack = inBlack || 0;
      gamma = gamma || 1;
      inWhite = (inWhite != null) ? inWhite : 255;
      outBlack = outBlack || 0;
      outWhite = (outWhite != null) ? outWhite : 255;
      if (inBlack >= inWhite) return; // invalid range
      _edEnsureEditsPx(); _edSaveUndo();
      const pw = _edCvsEditsPx.width, ph = _edCvsEditsPx.height;
      const ctx = _edCvsEditsPx.getContext("2d", { willReadFrequently: true });
      const imgData = ctx.getImageData(0, 0, pw, ph);
      ...
      for (let i = 0; i < d.length; i += 4) {
        ...
        for (let c = 0; c < 3; c++) {
          ...
          v = Math.max(0, Math.min(1, (v - inBlack) / range));
          // Apply gamma
    ...
    function openLevelsModal() {
      ...
      const pw = _edCvsEditsPx.width, ph = _edCvsEditsPx.height;
      const ctx = _edCvsEditsPx.getContext("2d", { willReadFrequently: true });
      const initialSnap = ctx.getImageData(0, 0, pw, ph);

      function sliderToGamma(x) {
        const g = Math.pow(10, (x - 100) / 100);
      ...
      `;
      title.innerHTML = `📊 Levels Adjustment`;
      ...
      function createSection(titleText) {
        ...
        `;
        secHeader.textContent = titleText;
        sec.appendChild(secHeader);
        return sec;
      }

      // Input Section
      const inSec = createSection("Input Levels");
      const inGrad = document.createElement("div");
      inGrad.style.cssText = `
        height: 6px;
        background: linear-gradient(to right, #000000 0%, #ffffff 100%);
        border-radius: 3px;
        border: 1px solid #27272a;
      `;
      inSec.appendChild(inGrad);

      function createSliderRow(label, min, max, val, isGamma) {
        const row = document.createElement("div");
        row.style.cssText = `
          display: flex;
          align-items: center;
        ...
        valSpan.style.cssText = `
          ...
          font-weight: 500;
        `;
        valSpan.textContent = isGamma ? sliderToGamma(val).toFixed(2) : val;
      ...
      const inBRow = createSliderRow("Shadows", "0", "253", "0", false);
      const gamRow = createSliderRow("Midtones", "0", "200", "100", true);
      const inWRow = createSliderRow("Highlights", "2", "255", "255", false);

      inSec.appendChild(inBRow.row);
      inSec.appendChild(gamRow.row);
      inSec.appendChild(inWRow.row);
      card.appendChild(inSec);

      // Output Section
      const outSec = createSection("Output Levels");
      const outGrad = document.createElement("div");
      outGrad.style.cssText = `
        height: 6px;
        background: linear-gradient(to right, #000000 0%, #ffffff 100%);
        border-radius: 3px;
        border: 1px solid #27272a;
      `;
      outSec.appendChild(outGrad);

      const outBRow = createSliderRow("Out Black", "0", "255", "0", false);
      const outWRow = createSliderRow("Out White", "0", "255", "255", false);

      outSec.appendChild(outBRow.row);
      outSec.appendChild(outWRow.row);
      card.appendChild(outSec);

      function updatePreview() {
        const inB = parseInt(inBRow.slider.value);
        const gam = sliderToGamma(parseInt(gamRow.slider.value));
        const inW = parseInt(inWRow.slider.value);
        const outB = parseInt(outBRow.slider.value);
        const outW = parseInt(outWRow.slider.value);

        ctx.putImageData(initialSnap, 0, 0);
        const imgData = ctx.getImageData(0, 0, pw, ph);
        const d = imgData.data;
        const range = Math.max(1, inW - inB);
        const invGamma = 1 / gam;
        ...
        for (let i = 0; i < d.length; i += 4) {
          ...
          for (let c = 0; c < 3; c++) {
            let v = d[i + c];
            v = Math.max(0, Math.min(1, (v - inB) / range));
            v = Math.pow(v, invGamma);
            v = outB + v * outRange;
            d[i + c] = Math.round(v);
          }
      ...
      inBRow.slider.addEventListener("input", (e) => {
        const val = parseInt(e.target.value);
        inBRow.valSpan.textContent = val;
        if (val >= parseInt(inWRow.slider.value)) {
          inWRow.slider.value = val + 2;
          inWRow.valSpan.textContent = val + 2;
        }
        updatePreview();
      });

      gamRow.slider.addEventListener("input", (e) => {
        gamRow.valSpan.textContent = sliderToGamma(parseInt(e.target.value)).toFixed(2);
        updatePreview();
      });

      inWRow.slider.addEventListener("input", (e) => {
        const val = parseInt(e.target.value);
        inWRow.valSpan.textContent = val;
        if (val <= parseInt(inBRow.slider.value)) {
          inBRow.slider.value = Math.max(0, val - 2);
          inBRow.valSpan.textContent = Math.max(0, val - 2);
        }
        updatePreview();
      });

      outBRow.slider.addEventListener("input", (e) => {
        outBRow.valSpan.textContent = e.target.value;
        updatePreview();
      });

      outWRow.slider.addEventListener("input", (e) => {
        outWRow.valSpan.textContent = e.target.value;
        updatePreview();
      });
      ...
      `;
      cancelB.addEventListener("mouseenter", () => cancelB.style.background = "#3f3f46");
      cancelB.addEventListener("mouseleave", () => cancelB.style.background = "#27272a");
      ...
      okB.style.cssText = `
        ...
        transition: background 0.12s;
      `;
      okB.addEventListener("mouseenter", () => okB.style.background = "#1d4ed8");
      okB.addEventListener("mouseleave", () => okB.style.background = "#2563eb");
      okB.addEventListener("click", () => {
        ...
        ctx.putImageData(initialSnap, 0, 0);
        _edApplyLevels(inB, gam, inW, outB, outW);
        document.body.removeChild(overlay);
      });

      btnRow.appendChild(cancelB);
      btnRow.appendChild(okB);
    ...
    globalUndoB.addEventListener("click", () => {
      if (edPanelMode === "pixels" && _edEditsUndoStack.length > 0) {
        _edUndoEdits();
      } else {
        _edUndoEditOps();
      }
    ...
    globalRedoB.addEventListener("click", () => {
      if (edPanelMode === "pixels" && _edEditsRedoStack.length > 0) {
        _edRedoEdits();
      } else {
        _edRedoEditOps();
      }
    ...
    function requestInpaintPreview() {
      ...
      const fn = items[curIdx]?.filename ?? "";
      const tSes = fn ? ses[fn] : null;
      if (_edCvsEditsPx) {
        if (tSes && tSes.imageEditsFile && (!_edEditsUndoStack || _edEditsUndoStack.length === 0)) {
          transform.imageEditsFile = tSes.imageEditsFile;
        } else {
          transform.imageEditsDataUrl = _edCvsEditsPx.toDataURL(edBg === "transparent" ? "image/png" : "image/webp", 0.92);
        }
      } else if (_edFlattenDataUrl) {
        if (tSes && tSes.imageEditsFile) {
          transform.imageEditsFile = tSes.imageEditsFile;
          transform.isFlattened = true;
        } else {
          transform.imageEditsDataUrl = _edFlattenDataUrl;
          transform.isFlattened = true;
        }
      }
    ...
    function redraw() {
      ...
      rafId = requestAnimationFrame(() => {
        ...
        if (isInpaint && edInpaintPreview && !edInpaintDirty && (typeof edPanelMode === "undefined" || edPanelMode !== "pixels")) {
          ...
          if (edImg) {
            ...
            if (_edBrushDrawing && edPixelTool === "brush" && !_edBrushErasing && _edActiveStrokeCvs) {
              ctx.save();
              ctx.globalAlpha = edBrushAlpha;
              ctx.imageSmoothingEnabled = true; ctx.imageSmoothingQuality = "high";
              ctx.drawImage(_edActiveStrokeCvs, -dw/2, -dh/2, dw, dh);
              ctx.restore();
            }
          ...
          if (isInpaint && edInpaintDirty && (typeof edPanelMode === "undefined" || edPanelMode !== "pixels")) {
    ...
    const _FH_R = Math.round(5 * uiScale);  // handle radius px
    const _FH_HIT = Math.round(10 * uiScale); // hit radius px
    ...
    function _getImgCorners() {
      ...
      // corners: [i,j] ∈ {-1,1}² — i=+1 is right, j=+1 is bottom
    ...
    // Cursor for a corner handle based on approximate angle
    ...
    function onGlobalMove(e) {
      ...
      if (_fhDrag) {
        ...
        const { corner, anchorX, anchorY, startMX, startMY,
                startScale, startSX, startSY, startDOX, startDOY } = _fhDrag;
        const altKey = e.altKey;   // Alt = from center
        const shiftKey = e.shiftKey; // Shift = distort (non-proportional)

        if (shiftKey) {
          // ── Distort mode: compute X and Y deltas in image-local space separately──
          const rad = edRotate * Math.PI / 180;
          const cosR = Math.cos(rad), sinR = Math.sin(rad);
          // Vector from anchor to current mouse in canvas space
          const dvx = mx - anchorX, dvy = my - anchorY;
          // Vector from anchor to start-handle in canvas space
          const dsx = startMX - anchorX, dsy = startMY - anchorY;
          // Project onto local (rotated) axes
          const dvLocalX =  dvx*cosR + dvy*sinR;
          const dvLocalY = -dvx*sinR + dvy*cosR;
          const dsLocalX =  dsx*cosR + dsy*sinR;
          const dsLocalY = -dsx*sinR + dsy*cosR;
          const ratioX = dsLocalX !== 0 ? dvLocalX / dsLocalX : 1;
          const ratioY = dsLocalY !== 0 ? dvLocalY / dsLocalY : 1;
          edScaleX = Math.max(0.05, startSX * ratioX);
          edScaleY = Math.max(0.05, startSY * ratioY);
        } else {
          // ── Proportional mode ──
          const dOrig = Math.hypot(startMX - anchorX, startMY - anchorY);
          const dNew  = Math.hypot(mx - anchorX, my - anchorY);
          const ratio = dOrig > 0.001 ? dNew / dOrig : 1;
          const newScale = Math.max(0.05, startScale * ratio);
          edScale = newScale;
          edScaleX = startSX; edScaleY = startSY; // keep relative distort
        }

        if (!altKey && !shiftKey) {
          // Anchor corner must stay fixed in canvas space after scale change
          // Recompute center so that anchor corner remains at (anchorX, anchorY)
          const { dw: ndw, dh: ndh } = _imgRenderDims();
          const { i: ai, j: aj } = _fhDrag.corner; // the DRAGGED corner's opposite = anchor
          const fX = edFlipH ? -1 : 1, fY = edFlipV ? -1 : 1;
          const alx = -ai*(ndw/2)*fX, aly = -aj*(ndh/2)*fY; // anchor local (opposite corner)
          const rad = edRotate * Math.PI / 180;
          const cxNew = anchorX - (alx*Math.cos(rad) - aly*Math.sin(rad));
          const cyNew = anchorY - (alx*Math.sin(rad) + aly*Math.cos(rad));
          dOX = cxNew - frameCX;
          dOY = cyNew - frameCY;
        } else if (!altKey && shiftKey) {
          // Distort: also keep anchor fixed
          const { dw: ndw, dh: ndh } = _imgRenderDims();
          const { i: ai, j: aj } = _fhDrag.corner;
          const fX = edFlipH ? -1 : 1, fY = edFlipV ? -1 : 1;
        ...
        // Alt: center is fixed — dOX/dOY unchanged
      ...
      if (edPixelTool && _edBrushDrawing) {
        ...
        const cx = e.clientX - r.left, cy = e.clientY - r.top;
        _edBrushPos = { cx, cy };
        const wpX = _edCvsEditsPx ? _edCvsEditsPx.width : 1;
        const wpY = _edCvsEditsPx ? _edCvsEditsPx.height : 1;
        const norm = cropPxToNorm(cx, cy);
        const epx = norm.nx * wpX;
        const epy = norm.ny * wpY;
        const { dw } = _imgRenderDims();
        const pxScale = _edCvsEditsPx ? wpX / dw : 1;
        ...
        if (edPixelTool === "blur") {
          ...
          if (_edBrushErasing) {
            ...
            // Draw continuous line segments onto _edActiveStrokeCvs with alpha=1.0 (no pile-up)
            if (!_edActiveStrokeCvs) _edActiveStrokeCvs = document.createElement("canvas");
            if (_edActiveStrokeCvs.width !== _edCvsEditsPx.width || _edActiveStrokeCvs.height !== _edCvsEditsPx.height) {
              _edActiveStrokeCvs.width = _edCvsEditsPx.width;
              _edActiveStrokeCvs.height = _edCvsEditsPx.height;
            }
            const sCtx = _edActiveStrokeCvs.getContext("2d");
            sCtx.lineCap = "round"; sCtx.lineJoin = "round";
            sCtx.globalCompositeOperation = "source-over";
            sCtx.strokeStyle = edColorFg; sCtx.fillStyle = edColorFg;
            sCtx.lineWidth = rPx * 2;
            if (prevPt) {
              if (edLassoOps.length > 0) {
                  const pw = _edCvsEditsPx.width, ph = _edCvsEditsPx.height;
                  const clipPath = _getLassoClipPath(pw, ph);
                  ...
                  if (edLassoInverted) {
                    const inv = new Path2D(); inv.rect(0, 0, pw, ph); inv.addPath(clipPath);
                    sCtx.clip(inv, "evenodd");
                  } else {
                    sCtx.clip(clipPath);
                  }
                  sCtx.beginPath(); sCtx.moveTo(prevPt.x, prevPt.y); sCtx.lineTo(epx, epy); sCtx.stroke();
                  sCtx.restore();
              } else {
                  sCtx.beginPath(); sCtx.moveTo(prevPt.x, prevPt.y); sCtx.lineTo(epx, epy); sCtx.stroke();
              }
              ...
              if (edLassoOps.length > 0) {
                  ...
                  sCtx.beginPath(); sCtx.arc(epx, epy, rPx, 0, Math.PI * 2); sCtx.fill();
                  sCtx.restore();
              } else {
                  sCtx.beginPath(); sCtx.arc(epx, epy, rPx, 0, Math.PI * 2); sCtx.fill();
              }
      ...
      if (edLassoMode) {
        ...
        if (edLassoTool === "freehand" && edLassoDrawing) {
          if (e.altKey) {
            // Alt = straight-line mode: keep only start + current endpoint
            edLassoCurrentPts = [edLassoCurrentPts[0], { x: cNx, y: cNy }];
          } else {
        ...
        if (edLassoTool === "polygonal" && edLassoCurrentPts.length > 0) {
          let snappedX = cNx, snappedY = cNy;
          if (e.altKey && edLassoCurrentPts.length >= 1) {
            // Alt = orthogonal snap: constrain to nearest 45° angle from last point
            const last = edLassoCurrentPts[edLassoCurrentPts.length - 1];
            const dx = cNx - last.x, dy = cNy - last.y;
            const angle = Math.atan2(dy, dx);
            const snapped = Math.round(angle / (Math.PI / 4)) * (Math.PI / 4);
            const dist = Math.hypot(dx, dy);
            snappedX = last.x + dist * Math.cos(snapped);
            snappedY = last.y + dist * Math.sin(snapped);
            snappedX = Math.max(0, Math.min(1, snappedX));
            snappedY = Math.max(0, Math.min(1, snappedY));
          }
          _lassoCursorNorm = { x: snappedX, y: snappedY }; redraw(); return;
        }
    ...
    function onGlobalUp(e) {
      ...
      if (edPixelTool && _edBrushDrawing) {
        if (edPixelTool === "brush" && !_edBrushErasing && _edActiveStrokeCvs) {
          const ctx = _edCvsEditsPx.getContext("2d");
          ctx.save();
          ctx.globalCompositeOperation = "source-over";
          ctx.globalAlpha = edBrushAlpha;
          ctx.drawImage(_edActiveStrokeCvs, 0, 0);
          ctx.restore();
        }
      ...
      if (edLassoMode && edLassoDrawing && edLassoTool === "freehand") {
        ...
        redraw(); ca.style.cursor = e.shiftKey ? _lassoCursors.add : _lassoCursors.normal; return;
      }
      if (edCropMode && edCropDrag) {
        edCropDrag = null;
        const r = cvs.getBoundingClientRect();
    ...
    cvs.addEventListener("mousedown", e=>{
      ...
      if (edPixelTool) {
        ...
        const wpY = _edCvsEditsPx ? _edCvsEditsPx.height : 1;
        const norm = cropPxToNorm(cx, cy);
        const startX = norm.nx * wpX;
        const startY = norm.ny * wpY;
        const { dw } = _imgRenderDims();
        const pxScale = _edCvsEditsPx ? wpX / dw : 1;
        ...
        if (_edAltEyedrop || edPixelTool === "eyedropper") {
          ...
          if (_edBrushErasing) {
            ...
            // Draw on active stroke offscreen canvas with full opacity to avoid "build up" overlapping during drag
            if (!_edActiveStrokeCvs) _edActiveStrokeCvs = document.createElement("canvas");
            if (_edActiveStrokeCvs.width !== _edCvsEditsPx.width || _edActiveStrokeCvs.height !== _edCvsEditsPx.height) {
              _edActiveStrokeCvs.width = _edCvsEditsPx.width;
              _edActiveStrokeCvs.height = _edCvsEditsPx.height;
            ...
            const sCtx = _edActiveStrokeCvs.getContext("2d");
            sCtx.clearRect(0, 0, _edActiveStrokeCvs.width, _edActiveStrokeCvs.height);
            sCtx.globalCompositeOperation = "source-over";
            sCtx.fillStyle = edColorFg;
            sCtx.strokeStyle = edColorFg;
            ...
            if (edLassoOps.length > 0) {
              ...
              sCtx.beginPath(); sCtx.arc(startX, startY, rPx, 0, Math.PI * 2); sCtx.fill();
              sCtx.restore();
            } else {
              sCtx.beginPath(); sCtx.arc(startX, startY, rPx, 0, Math.PI * 2); sCtx.fill();
            }
          ...
          if (_edCvsEditsPx) {
            _edCvsEditsPx.getContext("2d").globalCompositeOperation = "source-over";
          }
          redraw();
        }
        return;
      ...
      if (edLassoMode) {
        ...
        if (edLassoTool === "freehand") {
          ...
          // Use ortho-snapped point if Alt is held and _lassoCursorNorm has been set by mousemove
          const addX = (e.altKey && _lassoCursorNorm) ? _lassoCursorNorm.x : cNx;
          const addY = (e.altKey && _lassoCursorNorm) ? _lassoCursorNorm.y : cNy;
          edLassoCurrentPts.push({ x: addX, y: addY }); redraw();
        }
        return;
      }
      if (edCropMode) {
    ...
    cvs.addEventListener("mousemove", e=>{
      ...
      if (edLassoMode) {
        ca.style.cursor = e.shiftKey ? _lassoCursors.add : _lassoCursors.normal;
        return;
      }
      if (edCropMode && !edCropDrag) {
        const r = cvs.getBoundingClientRect();
    ...
    function onKey(e) {
      ...
      if ((e.ctrlKey || e.metaKey) && ((e.key === "z" && e.shiftKey) || e.key === "y")) {
        ...
        if (edPanelMode === "pixels" && _edEditsRedoStack.length > 0) { _edRedoEdits(); }
        else { _edRedoEditOps(); }
        return;
      }
      // Ctrl+Z: undo
      if ((e.ctrlKey || e.metaKey) && e.key === "z" && !e.shiftKey) {
        e.preventDefault(); e.stopImmediatePropagation();
        if (edPanelMode === "pixels" && _edEditsUndoStack.length > 0) { _edUndoEdits(); }
        else { _edUndoEditOps(); }
        return;
      }
      ...
      if (edPanelMode === "pixels" && !e.ctrlKey && !e.metaKey && !e.altKey) {
        ...
        if (k === "d") { edColorFg = "#ffffff"; edColorBg = "#000000"; edBrushAlpha = 1; localStorage.setItem("mil_fg_color", edColorFg); localStorage.setItem("mil_bg_color", edColorBg); localStorage.setItem("mil_brush_alpha", "1"); if(typeof ptFgPicker !== 'undefined') {ptFgPicker.value=edColorFg; ptBgPicker.value=edColorBg;} if(typeof ptLassoFillPick !== 'undefined') ptLassoFillPick.value=edColorFg; if(typeof ptAlphaSlider !== 'undefined') {ptAlphaSlider.value=100;} const _al=ptAlphaRow?.querySelector('span'); if(_al) _al.textContent='100%'; }
        if (k === "x") { const t = edColorFg; edColorFg = edColorBg; edColorBg = t; localStorage.setItem("mil_fg_color", edColorFg); localStorage.setItem("mil_bg_color", edColorBg); if(typeof ptFgPicker !== 'undefined') {ptFgPicker.value=edColorFg; ptBgPicker.value=edColorBg;} if(typeof ptLassoFillPick !== 'undefined') ptLassoFillPick.value=edColorFg; }
      }
  ...
  function openMaskEditor(startIdx, skipAnim) {
    ...
    const _savedAlpha = parseInt(localStorage.getItem("mil_mask_alpha") || "50");
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      const widgetEl = createWidget(node);
      const domWidget = node.addDOMWidget(
        "mil_uploader",
        "LoadImagesInGridWidget",
        widgetEl,
        {
          getValue() { return ""; },
          setValue() {},
          computeSize(width) {
            const computedHeight = Math.max(120, node.size[1] - NODE_HEADER_H - NODE_SLOT_H * 4 - NODE_PADDING_V);
      ...
      node.onDrop = async function (e) {
        ...
        try {
          const files = await filesFromDataTransfer(e.dataTransfer);
          if (files.length) {
            await node._milDomWidget?.element?._addFiles?.(files);
          }
        } catch (err) {
          console.warn("[MIL] node.onDrop failed:", err);
        }
        return false;
      };
